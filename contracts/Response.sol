...
contract Response is AccessControl {
    ...
    constructor(address _deployedOracle, address _deployedRequest) {
        ...
        request = Request(_deployedRequest);
    }
    ...
    // Get oracle manager address
    // function getOracleManagerAddress() public returns (address){
        ...
        )
    {
        ...
        // );
        require(responsesSet[_responseId].flag, "There is no request with the specified id");
        return (
    ...
    //     }
    //     return result;
        if (oracle.isRegistered(_sender) == true) return true;
    ...
    function createResponse(
        ...
        (bool _existance, uint256 _responseId, bool _responder) = requestHasResponse(requestId, oraclerAddress);
        require(
            _responder == false,
